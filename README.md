...
Pre-alpha. The first working core is available as a local Python CLI. It creates portable project state, enforces task gates, records rule approval, and provides a small local pre-commit baseline.
...
- Code cannot move to `implement` until the complete engineer brief, plan summary, named approval, and task ruleset snapshot are recorded.
- Rules are proposals until an engineer runs `anchor rules approve <id>`.
...
anchor add --apply
anchor rules list
anchor rules approve baseline-code-quality-v1
anchor rules approve baseline-security-v1
anchor rules approve baseline-structure-v1
...
anchor rules approve rule-structure-<id>
~~~
