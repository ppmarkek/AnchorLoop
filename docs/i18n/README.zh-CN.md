...
> - **已发布的 production：** `anchorloop@0.2.1`
> - **当前版本：** `0.2.1`
> - 自动化请使用精确版本：`npx --yes anchorloop@0.2.1 ...`。
...
- 生产版本 `0.2.1` 的 npm 包和独立 Python CLI。
...
```sh
npx --yes anchorloop@0.2.1 --help
npx --yes anchorloop@0.2.1 doctor
```

也可以安装到项目中：

```sh
npm install --save-dev anchorloop@0.2.1
npx anchorloop@0.2.1 init
```

如果项目使用锁文件，请提交 `package-lock.json` 的变化。自动化脚本应固定到 `0.2.1`，不要依赖浮动的版本范围。
...
```sh
python3 -m pip install anchorloop==0.2.1
anchor --help
anchor doctor
```
...
python3 -m venv .venv
. .venv/bin/activate
python -m pip install anchorloop==0.2.1
```
...
```sh
npm install --save-dev anchorloop@0.2.1
npx anchorloop@0.2.1 add --apply
npx anchorloop@0.2.1 doctor
```

如果使用 Python CLI：

```sh
python3 -m pip install --upgrade anchorloop==0.2.1
anchor add --apply
anchor doctor
```
...
node npm/scripts/finalize-release-docs.js --check --version 0.2.1 --release-date 2026-07-20
```
