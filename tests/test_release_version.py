...
class ReleaseVersionTests(unittest.TestCase):
    ...
    def test_release_workflow_stages_with_oidc_and_keeps_promotion_human_owned(self) -> None:
        ...
        self.assertIn("release[- ]candidate", finalizer)
        self.assertIn("published production", finalizer)
        self.assertIn("raw.githubusercontent.com/ppmarkek/AnchorLoop/v${version}/", finalizer)
        ...
        self.assertIn("post-promotion documentation PR against main", promotion)
        self.assertIn("status surface and its release-document tests", promotion)
        self.assertIn("${VERSION} candidate to ${VERSION} production", promotion)
        self.assertIn("finalizer source fragments and source-RC assertions", promotion)
    ...
    def test_release_documentation_stays_candidate_until_external_gates_complete(self) -> None:
        root = pathlib.Path(__file__).resolve().parents[1]
        ...
        self.assertIn("anchorloop@0.1.0", security)
        self.assertIn("0.2.0", security)
        self.assertIn("unreleased release", security.lower())
        self.assertIn("anchorloop@0.1.0` remains the published production baseline", contributing)
        ...
        self.assertIn("deterministically transforms only declared packaged documentation", contributing)
        self.assertIn("post-promotion documentation PR against `main`", contributing)
        self.assertIn("to `0.2.0` production", contributing)
        self.assertIn("finalized Git checkout directory", contributing)
        ...
        self.assertIn("with lifecycle scripts disabled", contributing)
        self.assertIn("finalizer source fragments and source-RC assertions", contributing)
        self.assertNotIn("publishes it with npm provenance through OIDC", contributing)
        self.assertNotIn("exact-source", contributing)
        self.assertIn("- Published production: `anchorloop@0.1.0`", migration)
        self.assertIn(
        ...
        self.assertIn("stage-only `npm stage publish`", normalized_decision_map)
        self.assertIn("stores no npm token", normalized_decision_map)
        self.assertNotIn("`anchorloop@0.2.0` is published production", decision_map)
        ...
        for translation in translations:
            ...
            self.assertIn("anchorloop@0.1.0", banner, translation.name)
            self.assertIn("`0.2.0`", banner, translation.name)
